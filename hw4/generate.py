...
tf.flags.DEFINE_string("samples_dir", "./samples/", "test image directory")
...
# tf.flags.DEFINE_string("model", "DCGAN", "init model name")
tf.flags.DEFINE_string("model", "ACGAN", "init model name")
tf.flags.DEFINE_boolean("prepro", False, "preprocessing the training data")
...
def main(_):
	...
	if FLAGS.prepro:
		img_feat, tags_idx, a_tags_idx, vocab_processor = data_utils.load_train_data(FLAGS.train_dir, FLAGS.tag_path, FLAGS.prepro_dir)	
	else:
		...
		vocab_processor = VocabularyProcessor.restore(os.path.join(FLAGS.prepro_dir, "vocab"))
	...
	model = Model(data, vocab_processor, FLAGS, test=True)
	
	model.build_model()

	model.load_model("./model/model-39500")
