...
class Discriminator(object):
    ...
    def __call__(self, tags_vectors, img, reuse=True):
        ...
        with tf.variable_scope("d_net") as scope:
            ...
            #     activation_fn=None
            #     )
            # tags_vectors = leaky_relu(tags_vectors)
            ...
            output = tc.layers.convolution2d(
                net_h4, 1, [s16, s16], [s16, s16],
                padding='valid',
                weights_initializer=tf.random_normal_initializer(stddev=0.02),
                activation_fn=None
                )
            output_class = tc.layers.convolution2d(
                net_h4, 26, [s16, s16], [s16, s16],
                padding='valid',
                weights_initializer=tf.random_normal_initializer(stddev=0.02),
                activation_fn=None
                )
            output_class = tf.squeeze(output_class)

            # output = tf.layers.dense(net_h4, 1)
            # # output = tf.squeeze(conv5, [1, 2, 3])
            # output_class = tf.layers.dense(net_h4, 26)
