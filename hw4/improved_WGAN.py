...
class Improved_WGAN(object):
	...
	def build_model(self):
		...
		self.d_loss = (tf.reduce_mean(self.d_1)+tf.reduce_mean(self.d_2)+tf.reduce_mean(self.d_3))/3. - tf.reduce_mean(self.d)

		alpha = tf.random_uniform([], 0.0, 1.0)
		interpolates = alpha * r_img + (1 - alpha) * self.f_img
		gradients = tf.gradients(self.d_net(r_seq, interpolates), interpolates)[0]
		gradients = tf.reshape(gradients, [-1, self.img_row * self.img_col * 3])
