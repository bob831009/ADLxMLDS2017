...
class Data(object):
	...
	def next_noise_batch(self, size, dim, random_state=-1):
		if(random_state == -1): return self.z_sampler.rvs([size, dim]) #np.random.uniform(-1.0, 1.0, [size, dim])
		else: return self.z_sampler.rvs([size, dim], random_state=random_state)
...
def load_train_data(img_dir, tag_path, prepro_dir, shuffle_time=1):
	...
	vocab_processor.save(os.path.join(prepro_dir, "vocab"))
...
def dump_img(img_dir, img_feats, iters):
	...
	for idx, img_feat in enumerate(img_feats):
		img_feat = misc.imresize(img_feat, [64, 64, 3])
...
def dump_test_img(img_dir, img_feats, sample_id):
	...
	for idx, img_feat in enumerate(img_feats):
		img_feat = misc.imresize(img_feat, [64, 64, 3])
		path = os.path.join(img_dir, 'sample_{}_{}.jpg'.format(idx+1, sample_id+1))
		misc.imsave(path, img_feat)
